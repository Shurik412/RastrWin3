...
def set_ut_common(
    ...
    get_maxs = get_.get_cell(table=ut_common_table,
                             column=ut_common_attributes_list[0],
                             row_id=0)
    variable_def_rowid.make_changes_row(table=ut_common_table,
                                        column=ut_common_attributes_list[0],
                                        row_id=0,
                                        value=float(maxs))
    get_maxs_after = get_.get_cell(table=ut_common_table,
                                   column=ut_common_attributes_list[0],
                                   row_id=0)

    get_maxv = get_.get_cell(table=ut_common_table,
                             column=ut_common_attributes_list[1],
                             row_id=0)
    variable_def_rowid.make_changes_row(table=ut_common_table,
                                        column=ut_common_attributes_list[1],
                                        row_id=0,
                                        value=float(maxv))
    get_maxv_after = get_.get_cell(table=ut_common_table,
                                   column=ut_common_attributes_list[1],
                                   row_id=0)

    get_maxd = get_.get_cell(table=ut_common_table,
                             column=ut_common_attributes_list[2],
                             row_id=0)
    variable_def_rowid.make_changes_row(table=ut_common_table,
                                        column=ut_common_attributes_list[2],
                                        row_id=0,
                                        value=float(maxd))
    get_maxd_after = get_.get_cell(table=ut_common_table,
                                   column=ut_common_attributes_list[2],
                                   row_id=0)

    get_maxa = get_.get_cell(table=ut_common_table,
                             column=ut_common_attributes_list[3],
                             row_id=0)
    variable_def_rowid.make_changes_row(table=ut_common_table,
                                        column=ut_common_attributes_list[3],
                                        row_id=0,
                                        value=float(maxa))
    get_maxa_after = get_.get_cell(table=ut_common_table,
                                   column=ut_common_attributes_list[3],
                                   row_id=0)

    get_iter = get_.get_cell(table=ut_common_table,
                             column=ut_common_attributes_list[4],
                             row_id=0)
    variable_def_rowid.make_changes_row(table=ut_common_table,
                                        column=ut_common_attributes_list[4],
                                        row_id=0,
                                        value=float(iter))
    get_iter_after = get_.get_cell(table=ut_common_table,
                                   column=ut_common_attributes_list[4],
                                   row_id=0)

    get_tip = get_.get_cell(table=ut_common_table,
                            column=ut_common_attributes_list[5],
                            row_id=0)
    variable_def_rowid.make_changes_row(table=ut_common_table,
                                        column=ut_common_attributes_list[5],
                                        row_id=0,
                                        value=float(tip))
    get_tip_after = get_.get_cell(table=ut_common_table,
                                  column=ut_common_attributes_list[5],
                                  row_id=0)

    get_f_ots = get_.get_cell(table=ut_common_table,
                              column=ut_common_attributes_list[6],
                              row_id=0)
    variable_def_rowid.make_changes_row(table=ut_common_table,
                                        column=ut_common_attributes_list[6],
                                        row_id=0,
                                        value=float(f_ots))
    get_f_ots_after = get_.get_cell(table=ut_common_table,
                                    column=ut_common_attributes_list[6],
                                    row_id=0)

    get_add_d = get_.get_cell(table=ut_common_table,
                              column=ut_common_attributes_list[7],
                              row_id=0)
    variable_def_rowid.make_changes_row(table=ut_common_table,
                                        column=ut_common_attributes_list[7],
                                        row_id=0,
                                        value=float(add_d))
    get_add_d_after = get_.get_cell(table=ut_common_table,
                                    column=ut_common_attributes_list[7],
                                    row_id=0)

    get_ekstr = get_.get_cell(table=ut_common_table,
                              column=ut_common_attributes_list[8],
                              row_id=0)
    variable_def_rowid.make_changes_row(table=ut_common_table,
                                        column=ut_common_attributes_list[8],
                                        row_id=0,
                                        value=float(ekstr))
    get_ekstr_after = get_.get_cell(table=ut_common_table,
                                    column=ut_common_attributes_list[8],
                                    row_id=0)

    get_kfc = get_.get_cell(table=ut_common_table,
                            column=ut_common_attributes_list[9],
                            row_id=0)
    variable_def_rowid.make_changes_row(table=ut_common_table,
                                        column=ut_common_attributes_list[9],
                                        row_id=0,
                                        value=float(kfc))
    get_kfc_after = get_.get_cell(table=ut_common_table,
                                  column=ut_common_attributes_list[9],
                                  row_id=0)

    get_sum_kfc = get_.get_cell(table=ut_common_table,
                                column=ut_common_attributes_list[10],
                                row_id=0)
    variable_def_rowid.make_changes_row(table=ut_common_table,
                                        column=ut_common_attributes_list[10],
                                        row_id=0,
                                        value=float(sum_kfc))
    get_sum_kfc_after = get_.get_cell(table=ut_common_table,
                                      column=ut_common_attributes_list[10],
                                      row_id=0)

    get_ds = get_.get_cell(table=ut_common_table,
                           column=ut_common_attributes_list[11],
                           row_id=0)
    variable_def_rowid.make_changes_row(table=ut_common_table,
                                        column=ut_common_attributes_list[11],
                                        row_id=0,
                                        value=float(ds))
    get_ds_after = get_.get_cell(table=ut_common_table,
                                 column=ut_common_attributes_list[11],
                                 row_id=0)

    get_it = get_.get_cell(table=ut_common_table,
                           column=ut_common_attributes_list[12],
                           row_id=0)
    variable_def_rowid.make_changes_row(table=ut_common_table,
                                        column=ut_common_attributes_list[12],
                                        row_id=0,
                                        value=float(it))
    get_it_after = get_.get_cell(table=ut_common_table,
                                 column=ut_common_attributes_list[12],
                                 row_id=0)

    get_status = get_.get_cell(table=ut_common_table,
                               column=ut_common_attributes_list[13],
                               row_id=0)
    variable_def_rowid.make_changes_row(table=ut_common_table,
                                        column=ut_common_attributes_list[13],
                                        row_id=0,
                                        value=float(Status))
    get_status_after = get_.get_cell(table=ut_common_table,
                                     column=ut_common_attributes_list[13],
                                     row_id=0)

    get_korr_t = get_.get_cell(table=ut_common_table,
                               column=ut_common_attributes_list[14],
                               row_id=0)
    variable_def_rowid.make_changes_row(table=ut_common_table,
                                        column=ut_common_attributes_list[14],
                                        row_id=0,
                                        value=float(KorrT))
    get_korr_t_after = get_.get_cell(table=ut_common_table,
                                     column=ut_common_attributes_list[14],
                                     row_id=0)

    get_korr_per = get_.get_cell(table=ut_common_table,
                                 column=ut_common_attributes_list[15],
                                 row_id=0)
    variable_def_rowid.make_changes_row(table=ut_common_table,
                                        column=ut_common_attributes_list[15],
                                        row_id=0,
                                        value=float(KorrPer))
    get_korr_per_after = get_.get_cell(table=ut_common_table,
                                       column=ut_common_attributes_list[15],
                                       row_id=0)

    get_korr_vib = get_.get_cell(table=ut_common_table,
                                 column=ut_common_attributes_list[16],
                                 row_id=0)
    variable_def_rowid.make_changes_row(table=ut_common_table,
                                        column=ut_common_attributes_list[16],
                                        row_id=0,
                                        value=float(KorrVib))
    get_korr_vib_after = get_.get_cell(table=ut_common_table,
                                       column=ut_common_attributes_list[16],
                                       row_id=0)

    get_enable_contr = get_.get_cell(table=ut_common_table,
                                     column=ut_common_attributes_list[17],
                                     row_id=0)
    variable_def_rowid.make_changes_row(table=ut_common_table,
                                        column=ut_common_attributes_list[17],
                                        row_id=0,
                                        value=float(enable_contr))
    get_enable_contr_after = get_.get_cell(table=ut_common_table,
                                           column=ut_common_attributes_list[17],
                                           row_id=0)

    get_dis_v_contr = get_.get_cell(table=ut_common_table,
                                    column=ut_common_attributes_list[18],
                                    row_id=0)
    variable_def_rowid.make_changes_row(table=ut_common_table,
                                        column=ut_common_attributes_list[18],
                                        row_id=0,
                                        value=float(dis_v_contr))
    get_dis_v_contr_after = get_.get_cell(table=ut_common_table,
                                          column=ut_common_attributes_list[18],
                                          row_id=0)

    get_dis_p_contr = get_.get_cell(table=ut_common_table,
                                    column=ut_common_attributes_list[19],
                                    row_id=0)
    variable_def_rowid.make_changes_row(table=ut_common_table,
                                        column=ut_common_attributes_list[19],
                                        row_id=0,
                                        value=float(dis_p_contr))
    get_dis_p_contr_after = get_.get_cell(table=ut_common_table,
                                          column=ut_common_attributes_list[19],
                                          row_id=0)

    get_dis_i_contr = get_.get_cell(table=ut_common_table,
                                    column=ut_common_attributes_list[20],
                                    row_id=0)
    variable_def_rowid.make_changes_row(table=ut_common_table,
                                        column=ut_common_attributes_list[20],
                                        row_id=0,
                                        value=float(dis_i_contr))
    get_dis_i_contr_after = get_.get_cell(table=ut_common_table,
                                          column=ut_common_attributes_list[20],
                                          row_id=0)

    get_ss_calc = get_.get_cell(table=ut_common_table,
                                column=ut_common_attributes_list[21],
                                row_id=0)
    variable_def_rowid.make_changes_row(table=ut_common_table,
                                        column=ut_common_attributes_list[21],
                                        row_id=0,
                                        value=float(ss_calc))
    get_ss_calc_after = get_.get_cell(table=ut_common_table,
                                      column=ut_common_attributes_list[21],
                                      row_id=0)

    get_criterion = get_.get_cell(table=ut_common_table,
                                  column=ut_common_attributes_list[22],
                                  row_id=0)
    variable_def_rowid.make_changes_row(table=ut_common_table,
                                        column=ut_common_attributes_list[22],
                                        row_id=0,
                                        value=float(criterion))
    get_criterion_after = get_.get_cell(table=ut_common_table,
                                        column=ut_common_attributes_list[22],
                                        row_id=0)

    get_no_crit_d_ba = get_.get_cell(table=ut_common_table,
                                     column=ut_common_attributes_list[23],
                                     row_id=0)
    variable_def_rowid.make_changes_row(table=ut_common_table,
                                        column=ut_common_attributes_list[23],
                                        row_id=0,
                                        value=float(no_crit_d_ba))
    get_no_crit_d_ba_after = get_.get_cell(table=ut_common_table,
                                           column=ut_common_attributes_list[23],
                                           row_id=0)

    get_no_crit_d_coa = get_.get_cell(table=ut_common_table,
                                      column=ut_common_attributes_list[24],
                                      row_id=0)
    variable_def_rowid.make_changes_row(table=ut_common_table,
                                        column=ut_common_attributes_list[24],
                                        row_id=0,
                                        value=float(no_crit_d_coa))
    get_no_crit_d_coa_after = get_.get_cell(table=ut_common_table,
                                            column=ut_common_attributes_list[24],
                                            row_id=0)

    get_no_crit_d_ga = get_.get_cell(table=ut_common_table,
                                     column=ut_common_attributes_list[25],
                                     row_id=0)
    variable_def_rowid.make_changes_row(table=ut_common_table,
                                        column=ut_common_attributes_list[25],
                                        row_id=0,
                                        value=float(no_crit_d_ga))
    get_no_crit_d_ga_after = get_.get_cell(table=ut_common_table,
                                           column=ut_common_attributes_list[25],
                                           row_id=0)

    get_save_files_filter = get_.get_cell(table=ut_common_table,
                                          column=ut_common_attributes_list[26],
                                          row_id=0)
    variable_def_rowid.make_changes_row(table=ut_common_table,
                                        column=ut_common_attributes_list[26],
                                        row_id=0,
                                        value=float(save_files_filter))
    get_save_files_filter_after = get_.get_cell(table=ut_common_table,
                                                column=ut_common_attributes_list[26],
                                                row_id=0)

    get_save_files_path = get_.get_cell(table=ut_common_table,
                                        column=ut_common_attributes_list[27],
                                        row_id=0)
    variable_def_rowid.make_changes_row(table=ut_common_table,
                                        column=ut_common_attributes_list[27],
                                        row_id=0,
                                        value=float(save_files_path))
    get_save_files_path_after = get_.get_cell(table=ut_common_table,
                                              column=ut_common_attributes_list[27],
                                              row_id=0)

    get_stop_u_n = get_.get_cell(table=ut_common_table,
                                 column=ut_common_attributes_list[28],
                                 row_id=0)
    variable_def_rowid.make_changes_row(table=ut_common_table,
                                        column=ut_common_attributes_list[28],
                                        row_id=0,
                                        value=float(stop_u_n))
    get_stop_u_n_after = get_.get_cell(table=ut_common_table,
                                       column=ut_common_attributes_list[28],
                                       row_id=0)

    get_dyn_find_pred = get_.get_cell(table=ut_common_table,
                                      column=ut_common_attributes_list[29],
                                      row_id=0)
    variable_def_rowid.make_changes_row(table=ut_common_table,
                                        column=ut_common_attributes_list[29],
                                        row_id=0,
                                        value=float(dyn_find_pred))
    get_dyn_find_pred_after = get_.get_cell(table=ut_common_table,
                                            column=ut_common_attributes_list[29],
                                            row_id=0)

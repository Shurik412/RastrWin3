...
from RastrWinLib.getting.get import Get
...
def change_parameters(
    ...
    if Id is not None:
        # Ku
        ku_before = get_.get_param(table=ExcControl.table,
...
def getting(Id=None, row_id=None):
    """
    :param Id:
    :param row_id:
    :return:
    ...
    #     """
    #
    #     def __init__(self, table, rastr_win=rastr_win):
    #         self.table_ = rastr_win.Tables(table)
    #
    #     def rget(self, column, Id):
    #         self.table_.SetSel(f'Id=1320')
    #         row_ = self.table_.FindNextSel(-1)
    #         val_ = self.table_.Cols(column).Z(row_)
    #         return val_

    get_ = Get(Id=1320, table=exc_cont.table, rastr_win=RASTR)

    if Id is not None:
        # Ku
        get_ku = Get(table='ExcControl', Id=Id, rastr_win=RASTR)
        Ku = get_.rget(column=exc_cont.Ku)
        # K_Q
        get_kq = Get(table='ExcControl', Id=Id, rastr_win=RASTR)
        K_Q = get_.rget(column=exc_cont.K_Q)
        # Kif1
        get_kif1 = Get(table='ExcControl', Id=Id, rastr_win=RASTR)
        Kif1 = get_.rget(column=exc_cont.Kif1)
        # T1if
        get_t1if = Get(table='ExcControl', Id=Id, rastr_win=RASTR)
        T1if = get_.rget(column=exc_cont.T1if)
        # Ku1
        get_ku1 = Get(table='ExcControl', Id=Id, rastr_win=RASTR)
        Ku1 = get_.rget(column=exc_cont.Ku1)
        # T1u1
        get_t1u1 = Get(table='ExcControl', Id=Id, rastr_win=RASTR)
        T1u1 = get_.rget(column=exc_cont.T1u1)
        # K_P
        get_k_p = Get(table='ExcControl', Id=Id, rastr_win=RASTR)
        K_P = get_.rget(column=exc_cont.K_P)
        # K_Ia
        get_k_ia = Get(table='ExcControl', Id=Id, rastr_win=RASTR)
        K_Ia = get_.rget(column=exc_cont.K_Ia)
        # Tf
        get_tf = Get(table='ExcControl', Id=Id, rastr_win=RASTR)
        Tf = get_.rget(column=exc_cont.Tf)
        # Kf
        get_kf = Get(table='ExcControl', Id=Id, rastr_win=RASTR)
        Kf = get_.rget(column=exc_cont.Kf)
        # Kf1
        get_kf1 = Get(table='ExcControl', Id=Id, rastr_win=RASTR)
        Kf1 = get_.rget(column=exc_cont.Kf1)
        # TINT
        get_tint = Get(table='ExcControl', Id=Id, rastr_win=RASTR)
        TINT = get_.rget(column=exc_cont.TINT)
        print(
            f'{separator_grid}\n'
    ...
    if row_id is not None:
        # Ku
        Ku = get_.get_cell(table=ExcControl.table,
                           column=ExcControl.Ku,
                           row_id=row_id)
        # K_Q
        K_Q = self.get_.get_cell(table=ExcControl.table,
                                 column=ExcControl.K_Q,
                                 row_id=row_id)
        # Kif1

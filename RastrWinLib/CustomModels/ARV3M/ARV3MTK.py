# -*- coding: utf-8 -*-
# Модель автоматического регулятора возбуждения типа ARV-3MTK,
# реализованная в ПК RUSTab, состоит из следующих макроблоков:
# 1.АРВ: AVR-3MTK_res.xmldev;
# 2.Релейная форсировка возбуждения: AVR-3MTK_RF.xmldev (таблица 8).
# Параметры модели AVR-3MTK_res.xmldev заносятся в таблицу «АРВ (ИД)».
...
# Параметры модели AVR-3MTK_RF.xmldev заносятся в таблицу «Форсировка (ИД)».
...
class Parameters:
    ...
    реализованная в ПК RUSTab, состоит из следующих макроблоков:
    1.АРВ: AVR-3MTK_res.xmldev;
    2.Релейная форсировка возбуждения: AVR-3MTK_RF.xmldev.
    Параметры модели AVR-3MTK_res.xmldev заносятся в таблицу «АРВ (ИД)».
    Типовые значение параметров настройки, приняты согласно информации от
    производителя системы возбуждения. В случае наличия листинга настроек для
    конкретного энергообъекта необходимо использовать фактические параметры настройки.

...
for i in range(0, max_col_gen_file_one):
    ...
    if row_gen_file_two != (-1):
        ...
        p_gen_gen_file_two = p_gen_obj_gen_file_two.make_changes_row(table=generator_table, column='P', row_id=i)

        p_gen_obj_gen_file_two = Variable(rastr_win=two_rastr, switch_command_line=False)
        p_gen_obj_gen_file_two.make_changes_row(table=generator_table,
                                                column='P',
        ...
        q_gen_obj_gen_file_two.make_changes_row(table=generator_table,
                                                column='Q',
                                                row_id=row_gen_file_two,
                                                value=q_gen_gen_file_one)

        sta_gen_obj_gen_file_two = Variable(rastr_win=two_rastr, switch_command_line=False)
        sta_gen_obj_gen_file_two.make_changes_row(table=generator_table,
                                                  column='sta',
                                                  row_id=row_gen_file_two,
        ...
        name_gen_file_two = name_obj_gen_file_two.make_changes_row(table=generator_table,
                                                                   column='Name',
                                                                   row_id=row_gen_file_two)

        p_gen_obj_gen_file_one_afte = Variable(rastr_win=two_rastr,
                                               switch_command_line=False)
        pg_afte = p_gen_obj_gen_file_one_afte.make_changes_row(table=generator_table,
                                                               column='P',
                                                               row_id=row_gen_file_two)
...
for j in range(0, max_col_vetv_file_one):
    # файл 1
    ip_vetv_file_one = get_.get_cell(table='vetv', column='ip', row_id=j)
    iq_vetv_file_one = get_.get_cell(table='vetv', column='iq', row_id=j)
    np_vetv_file_one = get_.get_cell(table='vetv', column='np', row_id=j)
    name_vetv_file_one = get_.get_cell(table='vetv', column='sta', row_id=j)
    sta_vetv_file_one = get_.get_cell(table='vetv', column='name', row_id=j)
    ...
    if row_vetv_file_two != (-1):
        ...
        sta_vetv_file_two_do = sta_vetv_obj_file_two_do.get_cell(table='vetv', column='sta', row_id=row_vetv_file_two)

        sta_vetv_obj_vetv_file_two = Variable(rastr_win=two_rastr, switch_command_line=False)
        sta_vetv_obj_vetv_file_two.make_changes_row(table='vetv',
                                                    column='sta',
                                                    row_id=row_vetv_file_two,
                                                    value=sta_vetv_file_one)

        sta_vetv_obj_file_two_posle = GettingParameter(rastr_win=two_rastr)
        sta_vetv_file_two_posle = sta_vetv_obj_file_two_posle.get_cell(table='vetv',
                                                                       column='sta',
                                                                       row_id=row_vetv_file_two)
...
for y in range(0, max_col_node_file_one):
    # файл 1
    ny_node_file_one = ny_obj_node_file_one.get_cell(table='node', column='ny', row_id=y)
    p_node_file_one = p_obj_node_file_one.get_cell(row_id=y, table='node', column='name')
    q_node_file_one = q_obj_node_file_one.get_cell(row_id=y, table='node', column='sta')
    name_node_file_one = name_obj_node_file_one.get_cell(row_id=y, table='node', column='vzd')
    sta_node_file_one = sta_obj_node_file_one.get_cell(row_id=y, table='node', column='pg')
    vzd_node_file_one = vzd_obj_node_file_one.get_cell(row_id=y, table='node', column='qg')
    ...
    if row_node_file_two != (-1):
        ...
        vzd_node_obj_file_two_posle = Variable(rastr_win=two_rastr,
                                               switch_command_line=False)
        vzd_node_file_two_posle = vzd_node_obj_file_two_posle.make_changes_row(table='node',
                                                                               column='vzd',
                                                                               row_id=row_node_file_two)
